...
def conv_format(y, X_logistic, file_list):
    # Print Info 
    ...
    file_list_df = pd.DataFrame(file_list, columns =["files"])
    
    return y_reshape, label_names_df, X_logistic_df, file_list_df

# > Save data 
def save(X_logistic_df, y_reshape, label_names_df, file_list_df):
    
    # > Save logistic regression data
    # Print info 
    print("[INFO] Saving logistic regression data...")
    # Get outpath 
    outpath_lr = os.path.join("in", "processed_data", "lr_data.csv")
    # Save data
    X_logistic_df.to_csv(outpath_lr, index=False)
    
    # > Save neural network data 
    # Print info 
    print("[INFO] Saving list of images...")
    # Save file list 
    outpath_fl = os.path.join("in", "processed_data", "file_list.csv")
    # Save data
    file_list_df.to_csv(outpath_fl, index=False)
...
def main():
    ...
    y_clean = clean_y(y)
    # Reshape data 
    X_logistic, file_list = reshape_data(y_clean, int(args["average"]), int(args["size"]), int(args["window"]))
    # Convert to appropriate formats 
    y_reshape, label_names_df, X_logistic_df, file_list_df = conv_format(y, X_logistic, file_list)
    # Save data 
    save(X_logistic_df, y_reshape, label_names_df, file_list_df)
